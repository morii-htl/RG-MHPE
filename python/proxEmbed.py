...
from proxEmbedProcessAndAssess import load_params
...
def adadelta(lr, tparams, grads, fourPairs, subPaths_matrix, subPaths_mask, subPaths_lens, groups_tensor,path_discount,wemb, cost):
    """
    ...
    f_grad_shared = theano.function([fourPairs, subPaths_matrix, subPaths_mask, subPaths_lens, groups_tensor,path_discount,wemb], cost, updates=zgup + rg2up,
    ...
    f_update = theano.function([lr], [], updates=ru2up + param_up,
                               on_unused_input='ignore',
                               name='adadelta_f_update') #这里的lr根本没有参与运算
...
def proxEmbedTraining(
    ...
    if wordsEmbeddings is None: 
        if wordsEmbeddings_path is not None: 
            wordsEmbeddings,dimension,wordsSize=dataProcessTools.getWordsEmbeddings(wordsEmbeddings_path)#嵌入矩阵(numpy)，维度，和节点数
            print(wordsEmbeddings.shape)
        else: 
    ...
    if subpaths_map is None:
        if subpaths_file is not None: 
            subpaths_map=dataProcessTools.loadAllSubPaths(subpaths_file, maxlen_subpaths)#一个字典 key：0-1 values：[[句子1]，[句子2]，[句子3]]
        else: 
            print 'There is not path for sub-paths, exit!!!'
            exit(0)
    
    trainingData,trainingPairs=dataProcessTools.getTrainingData(trainingDataFile) #前者是[[2,3,4],[4,5,6]]的包含所有训练数据的list，后者是对应的pair[[2-3,3-2,2-4,4-2],[4-5,5-4,4-6,6-4]]
    allBatches=dataProcessTools.get_minibatches_idx(len(trainingData), batch_size, is_shuffle_for_batch)
    params=init_sharedVariables(model_options) #初始化lstm的参数
    tparams=init_tparams(params) #设置成共享变量
    print 'Generate models ......'
    
    trainingParis, subPaths_matrix, subPaths_mask, subPaths_lens, groups_tensor,path_discount,wemb, cost=proEmbedModelBatch.proxEmbedModel(model_options, tparams) #只是一个算式，还没给数据
    print 'Generate gradients ......'
    grads=tensor.grad(cost,wrt=list(tparams.values()))
    print 'Using Adadelta to generate functions ......'
    lr = tensor.scalar(name='lr')
    f_grad_shared, f_update=adadelta(lr, tparams, grads, trainingParis, subPaths_matrix, subPaths_mask, subPaths_lens,groups_tensor,path_discount, wemb, cost)
    ...
    for eidx in range(max_epochs):
        for _, batch in allBatches: 
            ...
            triples_matrix_data, subPaths_matrix_data, subPaths_mask_data, subPaths_lens_data,groups_tensor,path_discount=dataProcessTools.prepareDataForTraining(trainingDataForBatch, trainingPairsForBatch, subpaths_map,model_options['discount_alpha'])
            group1=groups_tensor[0]
            if(triples_matrix_data.shape[0]==0):
                continue
            cost=0
            cost=f_grad_shared(triples_matrix_data, subPaths_matrix_data, subPaths_mask_data, subPaths_lens_data,groups_tensor,path_discount,wordsEmbeddings)   #
            f_update(lrate)
...
def proxEmbedTraining2(
    ...
    if subpaths_map is None:
        if subpaths_file is not None:
            subpaths_map = dataProcessTools.loadAllSubPaths(subpaths_file,
                                                            maxlen_subpaths)  # 一个字典 key：0-1 values：[[句子1]，[句子2]，[句子3]]
        else:
            print 'There is not path for sub-paths, exit!!!'
            exit(0)

    trainingData, trainingPairs = dataProcessTools.getTrainingData(
        trainingDataFile)  # 前者是[[2,3,4],[4,5,6]]的包含所有训练数据的list，后者是对应的pair[[2-3,3-2,2-4,4-2],[4-5,5-4,4-6,6-4]]
    allBatches = dataProcessTools.get_minibatches_idx(len(trainingData), batch_size, is_shuffle_for_batch)
    tparams = OrderedDict()
    tparams['lstm_W'] = None
    tparams['lstm_U'] = None
    tparams['lstm_b'] = None
    tparams['w'] = None
    tparams = load_params(saveto, tparams)
    tparams=init_tparams(tparams)
    print 'Generate models ......'

    trainingParis, subPaths_matrix, subPaths_mask, subPaths_lens,groups_tensor, wemb, cost = proEmbedModelBatch.proxEmbedModel(
        model_options, tparams)  # 只是一个算式，还没给数据

    print 'Generate gradients ......'
    grads = tensor.grad(cost, wrt=list(tparams.values()))
    print 'Using Adadelta to generate functions ......'
    lr = tensor.scalar(name='lr')
    f_grad_shared, f_update = adadelta(lr, tparams, grads, trainingParis, subPaths_matrix, subPaths_mask, subPaths_lens,groups_tensor,
                                       wemb, cost)
    ...
    for eidx in range(max_epochs):
        for _, batch in allBatches:
            ...
            trainingDataForBatch = [trainingData[i] for i in batch]
            trainingPairsForBatch = [trainingPairs[i] for i in batch]
            triples_matrix_data, subPaths_matrix_data, subPaths_mask_data, subPaths_lens_data,groups_tensor = dataProcessTools.prepareDataForTraining(
                trainingDataForBatch, trainingPairsForBatch, subpaths_map)
            print("fuck2")
            print(groups_tensor[0].shape)
            print(groups_tensor.shape)
            print(groups_tensor[0][:, :, None])
            if (triples_matrix_data.shape[0] == 0):
                continue
            cost = 0
            cost = f_grad_shared(triples_matrix_data, subPaths_matrix_data, subPaths_mask_data, subPaths_lens_data,groups_tensor,
                                 wordsEmbeddings)  #
            f_update(lrate)
